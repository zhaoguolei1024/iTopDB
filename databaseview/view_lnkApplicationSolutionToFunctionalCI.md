...
```
SELECT DISTINCT
	`lnkApplicationSolutionToFunctionalCI`.`id` AS `id`,
	...
	cast(
		concat(
			COALESCE ( `lnkApplicationSolutionToFunctionalCI`.`applicationsolution_id`, '' ),
			COALESCE ( ' ', '' ),
		COALESCE ( `lnkApplicationSolutionToFunctionalCI`.`functionalci_id`, '' )) AS CHAR charset utf8mb4 
	) AS `friendlyname`,
...
FROM
	((
			`lnkapplicationsolutiontofunctionalci` `lnkApplicationSolutionToFunctionalCI`
			JOIN (
				...
				JOIN `functionalci` `ApplicationSolution_applicationsolution_id_FunctionalCI` ON ((
					...
					`lnkApplicationSolutionToFunctionalCI`.`applicationsolution_id` = `ApplicationSolution_applicationsolution_id`.`id` 
				)))
		JOIN ((((((((
			...
			LEFT JOIN `physicaldevice` `FunctionalCI_functionalci_id_poly_PhysicalDevice` ON ((
				...
				`lnkApplicationSolutionToFunctionalCI`.`functionalci_id` = `FunctionalCI_functionalci_id`.`id` 
			))) 
WHERE
	((
