...
```
SELECT DISTINCT
	`lnkFunctionalCIToService`.`id` AS `id`,
	`lnkFunctionalCIToService`.`service_id` AS `service_id`,
	`Service_service_id`.`name` AS `service_name`,
	`lnkFunctionalCIToService`.`functionalci_id` AS `functionalci_id`,
	`FunctionalCI_functionalci_id`.`name` AS `functionalci_name`,
	cast(
		concat(
			COALESCE ( `lnkFunctionalCIToService`.`service_id`, '' ),
			COALESCE ( ' ', '' ),
		COALESCE ( `lnkFunctionalCIToService`.`functionalci_id`, '' )) AS CHAR charset utf8mb4 
	) AS `friendlyname`,
...
FROM
	((
			`lnkfunctionalcitoservice` `lnkFunctionalCIToService`
			JOIN `service` `Service_service_id` ON ((
					`lnkFunctionalCIToService`.`service_id` = `Service_service_id`.`id` 
				)))
		JOIN ((((((((
			...
			LEFT JOIN `physicaldevice` `FunctionalCI_functionalci_id_poly_PhysicalDevice` ON ((
				...
				`lnkFunctionalCIToService`.`functionalci_id` = `FunctionalCI_functionalci_id`.`id` 
			))) 
WHERE
	((
